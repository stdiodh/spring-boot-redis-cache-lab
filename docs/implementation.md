...
5. 수정/삭제 성공 후 해당 캐시를 evict하는 흐름을 확인합니다.
...
이 evict 연결은 현재 답안에 포함되어 있습니다. DB 쓰기가 성공한 뒤 캐시를 제거해야 실패한 쓰기 때문에 정상 캐시가 먼저 사라지지 않습니다.

### 확인 방법

`PostControllerCacheInvalidationTest`에서 수정/삭제 성공 후 `evict(id)`가 호출되는지 확인하고, TTL과 evict가 각각 자동 만료와 즉시 정리라는 다른 문제를 다룬다는 점을 설명합니다.
