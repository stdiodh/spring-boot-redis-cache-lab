...
이번 시퀀스는 반복 조회 앞에 Redis 캐시를 두고 cache miss, cache hit, TTL을 구현하며 수정/삭제 성공 후 해당 key를 evict합니다.
...
- `src/main/kotlin/com/andi/rest_crud/service/PostCacheService.kt`: cache get, set, evict, key, TTL 책임을 둡니다.
- `src/main/kotlin/com/andi/rest_crud/service/PostQueryService.kt`: cache miss와 cache hit 흐름을 읽는 조회 서비스입니다.
- `src/main/kotlin/com/andi/rest_crud/controller/PostController.kt`: 수정/삭제가 성공한 뒤 `PostCacheService.evict(id)`를 호출합니다.
- `src/main/kotlin/com/andi/rest_crud/service/PostService.kt`: DB 쓰기와 작성자 검증을 담당하고 캐시 세부 구현에는 의존하지 않습니다.
...
단위 테스트에서는 cache miss 때 DB를 읽는지, cache hit 때 DB 조회를 줄이는지 확인합니다. `PostControllerCacheInvalidationTest`는 수정/삭제 성공 뒤 캐시를 제거하는 순서를 검증합니다.
